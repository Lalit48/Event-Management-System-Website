...
// Handle delete request for offline events
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['event_id']) && isset($_GET['event_type']) && $_GET['event_type'] == 'offline') {
    $eventId = $_GET['event_id'];
    $sql_delete_offline = "DELETE FROM offline_events WHERE offline_event_id = $eventId";
    if (mysqli_query($conn, $sql_delete_offline)) {
        echo "<p>Offline event deleted successfully.</p>";
    } else {
        echo "<p>Error deleting offline event: " . mysqli_error($conn) . "</p>";
    }
}

// Handle delete request for online events
if (isset($_GET['action']) && $_GET['action'] == 'delete' && isset($_GET['event_id']) && isset($_GET['event_type']) && $_GET['event_type'] == 'online') {
    $eventId = $_GET['event_id'];
    
    // Prepare a delete statement
    $sql_delete_online = "DELETE FROM online_events WHERE online_event_id = ?";
    $stmt = mysqli_prepare($conn, $sql_delete_online);
    ...
    if (mysqli_stmt_execute($stmt)) {
        echo "<p>Online event deleted successfully.</p>";
    } else {
        echo "<p>Error deleting online event: " . mysqli_error($conn) . "</p>";
    }
...
while ($row = mysqli_fetch_assoc($result_offline)) {
    ...
    // Add delete button for offline event
    echo '<a href="events.php?action=delete&event_id=' . $row['offline_event_id'] . '&event_type=offline" class="btn btn-danger">Delete</a>';
...
while ($row = mysqli_fetch_assoc($result_online)) {
    ...
    echo '<p><strong>Date & Time:</strong> ' . $row['dateTime'] . '</p>';
    // Add delete button for online event
    echo '<a href="events.php?action=delete&event_id=' . $row['online_event_id'] . '&event_type=online" class="btn btn-danger">Delete</a>';
    echo '</div>'; // Close content div
    echo '</div>'; // Close box div
}
